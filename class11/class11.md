...
- [Current Limitations and Potential
  Problems](#current-limitations-and-potential-problems)
- [Summary](#summary)

## Background

We saw last day that the main repository for biomolecular strcuture (the
...
work to determine the structure of just one protien. With ALphaFold we
can now accurately compute a typical protein structure in as little as
10 minutes.
...
Examining the RMSF between positions of the structure. RMSF is used as a
...
We can see that the first chain is very similar across different models,
but the second chain is more variable, which was also visualized in Mol.

## Predicted Alignment Error for Domains

AlphaFold produces an output called **Predicted Aligned Error (PAE)**
which is independent of the 3D structure.
...
Note the conserved Active Site residues D25, T26, G27, A28, which we can
see if we generate a consensus sequence with a high cutoff value:
...
We can view these functionally important sites by mapping the
conservation score to the Occupancy column of a PDB file to view in
programs like MolStar.
...
1.  Multimer vs single chain predictiions: doesn’t always give
    reasonable results.
2.  Predicting different conformational states: can only predict a
    single state and is hard to tell which state of a protein will be
    captured.
3.  Relative positions of domains: if a predicted model has more than
    one domain, each domain may have high confidence, but the relative
    positions of the domains may not.
4.  Poorly predicted regions/bugs: there are protein regions that
    AlphaFold cannot predict.

## Summary

AlphaFold provides biologists with a new technique of structure-gazing
without the experimental structure determination work. As the next
generation of biologists, it is important we learn how to critically
analyze predicted structures, notice new interactions, and examine
proteins of interest in sufficient detail to differentiate between what
are “bugs” and what are “features”.
